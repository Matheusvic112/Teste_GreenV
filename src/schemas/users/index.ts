...
export const UserSchema = Yup.object().shape({
  ...
  phone: Yup.string()
    .matches(
      /^\(\d{2}\)\s\d{4,5}\-\d{4}$/,
      "Digite um telefone válido (99) 89999-9999"
    )
...
export const VeiculoSchema = Yup.object().shape({
  brand: Yup.string()
    ...
    .max(100, "A marca deve ter no máximo 50 caracteres"),
  model: Yup.string().required("O modelo é obrigatório"),
  year: Yup.string()
    .required("O Ano do veiculo é obrigatório")
    .max(4, "O ano deve ter no máximo 4 caracteres"),
...
export const CompanySchema = Yup.object().shape({
  ...
  phone: Yup.string()
    ...
    .required("O telefone é obrigatório"),
  cnpj: Yup.string()
    .required("Cnpj da companhia é obrigátorio")
