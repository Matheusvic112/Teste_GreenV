...
interface IUser {
 ...
 class UserService {
  ...
  async deleteUserById(userId: string) {
    const userDelete = await this.userRepo.findOne({ where: { id: userId }, relations: ["veiculos", "companies"]
  });
  if(!userDelete){
    return "Error"
  }
  ...
  async addUserToCompany(userId: string, companyId: string) {
    ...
    const findUser = await userRepository.findOne({ where: { id: userId }, relations: ["companies"]});
  
    const company = await companiesRepository.findOne({where:{id:companyId}});
