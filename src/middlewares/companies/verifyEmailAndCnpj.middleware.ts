...
export const emailAndCnpjMiddleware = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (email) {
    const existingEmailCompany = await companiesRepository.findOne({
      where: { email },
    })
  ...
  if (name) {
    const existingNomeCompany = await companiesRepository.findOne({
      where: { name },
    })
  ...
  if (cnpj) {
    const existingCnpjCompany = await companiesRepository.findOne({
      where: { cnpj },
    })
  ...
  if (phone) {
    const existingPhoneCompany = await companiesRepository.findOne({
      where: { phone },
    })
